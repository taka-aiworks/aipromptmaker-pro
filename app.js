...
// 共有: 服色と服名をペア化（順序保持・単独色掃除・DOM色優先・ワンピ最優先）
function pairWearColors(parts){
  if (!Array.isArray(parts)) return parts || [];

  // 元配列の順序を尊重して加工（文字列化＆空除去）
  let list = parts.filter(Boolean).map(s => String(s));

  // ---- 正規表現（色・名詞）
  const COLOR_WORD = '(white|black|red|blue|green|azure|yellow|pink|purple|brown|beige|gray|grey|silver|gold|navy|teal|cyan|magenta|orange)';
  const COLORS_RE  = new RegExp(`\\b${COLOR_WORD}\\b`, 'i');

  // NOTE: 先頭の (plain\\s+)? は検出用で、実際の出力では plain は剥がす
  ...
  const domTop    = domText('tag_top');
  const domBottom = domText('tag_bottom');
  const domShoes  = domText('tag_shoes');

  // list 内の「色トークン」のインデックス列挙（裸の色語／色+generic どちらも含む）
  const colorIdxs = [];
  ...
  // ユーティリティ
  const usedColor = new Set();          // 使い切った色トークンの index
  const usedToken = new Set();          // 後で削除したいトークン index（generic等）

  const nounOnly = (s, re) => {         // "plain " を剥いで名詞だけ返す
    const m = String(s).match(re);
    return m ? m[2].toLowerCase() : "";
  };

  // "white top" のような色＋generic を優先消費（見つかったら index を返す）
  const findPlaceholder = (generic) => {
    const re = new RegExp(`\\b${COLOR_WORD}\\s+${generic}\\b`, 'i');
    const i = list.findIndex(t => re.test(String(t)));
    if (i >= 0) return { idx: i, color: String(list[i]).match(re)[1].toLowerCase() };
    return null;
  };

  // 指定 generic の色を決める：DOM ＞ プレースホルダ ＞ 未使用の裸色
  const takeColorFor = (generic, domColor) => {
    if (domColor && COLORS_RE.test(domColor)) return domColor.match(COLORS_RE)[1].toLowerCase();

    const ph = findPlaceholder(generic);
    if (ph) { usedColor.add(ph.idx); return ph.color; }
  ...
  // list[nounIndex] を "color noun" or "noun" に置換（plain は常に剥ぐ）
  const combineAt = (nounIndex, re, colorWord) => {
    if (nounIndex < 0) return;
    const noun = nounOnly(list[nounIndex], re);
    list[nounIndex] = colorWord ? `${colorWord} ${noun}` : noun;
  };

  // ---------- ワンピ最優先 ----------
  if (idx.dress >= 0) {
    // ワンピの色は「トップ色（DOM）＞ 'color dress' プレース＞裸色」の順
    const colorDress = takeColorFor('dress', domTop || "");
    combineAt(idx.dress, DRESS_RE, colorDress);

    // ワンピ採用時は top/bottom 系の名詞とその generic プレースは削除
    const DROP_GENERIC_RE = /\b(?:top|bottom)\b/i;

    list = list.filter((t, i) => {
      // 既に使用済み色は削除
      if (usedColor.has(i)) return false;
      // トップ／ボトムの名詞は落とす
      if (TOP_RE.test(t) || BOTTOM_RE.test(t)) return false;
      // "white top" / "azure bottom" のような generic プレースも落とす
      if (new RegExp(`\\b${COLOR_WORD}\\s+(top|bottom)\\b`, 'i').test(t)) return false;
      // 裸の色単語（スペースなし）は落とす
      if (COLORS_RE.test(t) && !/\s/.test(String(t))) return false;
      // "plain ◯◯" が残っていたら plain を剥いだ名詞に正規化
      if (/^\s*plain\s+/i.test(t)) {
        const noPlain = t.replace(/^\s*plain\s+/i, '');
        return (list[i] = noPlain), true;
      }
      // "top" / "bottom" だけの素片は念のため落とす
      if (DROP_GENERIC_RE.test(t) && !/\s/.test(String(t))) return false;
    ...
    // ---------- 上下コーデ ----------
    if (idx.top >= 0) {
      const colorTop = takeColorFor('top', domTop);
      combineAt(idx.top, TOP_RE, colorTop);
    }
    if (idx.bottom >= 0) {
      const colorBottom = takeColorFor('bottom', domBottom);
      combineAt(idx.bottom, BOTTOM_RE, colorBottom);
    }

    // 靴は独立して処理
    if (idx.shoes >= 0) {
      const colorShoes = takeColorFor('shoes', domShoes);
      combineAt(idx.shoes, SHOES_RE, colorShoes);
    }

    // 後処理：使い切った色トークンや裸の色語、genericプレース等を掃除
    list = list.filter((t, i) => {
      // 使用済み色トークンは消す
      if (usedColor.has(i)) return false;
      // 裸の色単語は消す（"white" のように単独）
      if (COLORS_RE.test(t) && !/\s/.test(String(t))) return false;
      // "plain ◯◯" は plain を剥がして残す
      if (/^\s*plain\s+/i.test(t)) { list[i] = t.replace(/^\s*plain\s+/i, ''); return true; }
      // "white top/bottom/shoes" の generic プレースは全て消す（すでに名詞へ結合済み）
      if (new RegExp(`\\b${COLOR_WORD}\\s+(top|bottom|shoes|dress)\\b`, 'i').test(t)) return false;
      // "top" / "bottom" / "shoes" / "dress" 単体の素片も消す
      if (/\b(?:top|bottom|shoes|dress)\b/i.test(t) && !/\s/.test(String(t))) return false;

      return true;
    });
  }

  // 重複を軽く除去（順序保持のため Set 化→配列化）
  const out = [];
  const seen = new Set();
  for (const t of list){
    const k = t;
...
function pairWearColors(parts){
  ...
  const cleanNoun = (s, re) => {
    const m = String(s).match(re);
    if (!m) return "";
    return m[2].toLowerCase(); // "plain " は落とす
  };
...
function pmBuildOne(){
  ...
  // 基本情報（固定）
  const basics = [
    pickOneFromScroller('bf_age'),
    pickOneFromScroller('bf_gender'),
    pickOneFromScroller('bf_body'),
    pickOneFromScroller('bf_height'),
    pickOneFromScroller('hairStyle'),
    pickOneFromScroller('eyeShape'),
    textOf('tagH'), textOf('tagE'), textOf('tagSkin')
  ].filter(Boolean).map(norm);
  p.push(...basics);

  // 撮影で確定のシーン
  const bg   = pickOneFromScroller('pl_bg');
  const pose = pickOneFromScroller('pl_pose');
  const comp = pickOneFromScroller('pl_comp');
  const view = pickOneFromScroller('pl_view');
  const exprS= pickOneFromScroller('pl_expr');
  const liteS= pickOneFromScroller('pl_light');

  // 服：名詞 → 色プレース → ペアリング
  p.push(...getOutfitNouns());
  injectWearColorPlaceholders(p);
  ...
  let chosenNSFW = [];
  if (nsfwOn){
    const expo = pickOneFromScroller('pl_nsfw_expo');
    const und  = pickOneFromScroller('pl_nsfw_underwear');
    const outf = pickOneFromScroller('pl_nsfw_outfit');
    const ex2  = pickOneFromScroller('pl_nsfw_expr');
    const situ = pickOneFromScroller('pl_nsfw_situ');
    const li2  = pickOneFromScroller('pl_nsfw_light');
    const po2  = pickOneFromScroller('pl_nsfw_pose');
    const ac2  = pickOneFromScroller('pl_nsfw_acc');
    const bo2  = pickOneFromScroller('pl_nsfw_body');
    const nip  = pickOneFromScroller('pl_nsfw_nipple');
    ...
    // 露出/下着/衣装 選択で SFW服を掃除
    if (expo || und || outf){
      p = stripSfwWearWhenNSFW(p, outfitFixed);
    }

    // NSFW側があれば SFWの表情/光より優先
    if (ex2)  expr = norm(ex2);
    if (li2)  lite = norm(li2);

    chosenNSFW = [expo, und, outf, situ, po2, ac2, bo2, nip].filter(Boolean).map(norm);
  }

  // シーン確定
  p.push(...[bg, comp, pose, view, expr, lite].filter(Boolean).map(norm));
...
function buildBatchLearning(n){
  ...
  for (let i=0;i<wantCount;i++){
    ...
    // 学習でユーザーが選んだ“シーン系”
    const bg   = pickOneFromScroller('bg');
    const pose = pickOneFromScroller('pose');
    const comp = pickOneFromScroller('comp');
    const view = pickOneFromScroller('view');
    const expr = pickOneFromScroller('expr');
    const lite = pickOneFromScroller('lightLearn');

    p.push(...[bg, pose, comp, view, expr, lite].filter(Boolean).map(norm));
    ...
    let chosenNSFW = [];
    if (nsfwOn){
      const expo = pickOneFromScroller('nsfwL_expo');
      const und  = pickOneFromScroller('nsfwL_underwear');
      const outf = pickOneFromScroller('nsfwL_outfit');
      const ex2  = pickOneFromScroller('nsfwL_expr');
      const situ = pickOneFromScroller('nsfwL_situ');
      const li2  = pickOneFromScroller('nsfwL_light');
      const po2  = pickOneFromScroller('nsfwL_pose');
      const ac2  = pickOneFromScroller('nsfwL_acc');
      const bo2  = pickOneFromScroller('nsfwL_body');
      const nip  = pickOneFromScroller('nsfwL_nipple');

      // 先に pair後の「色+名詞」を保護集合化
      const outfitFixed = new Set(
        p.filter(s => /\b(white|black|red|blue|green|azure|yellow|pink|purple|brown|beige|gray|grey|silver|gold|navy|teal|cyan|magenta|orange)\b\s+\S/.test(String(s)))
         .map(norm)
      );

      // 露出/下着/衣装 を選んだら SFW服は掃除
      if (expo || und || outf){
        p = stripSfwWearWhenNSFW(p, outfitFixed);
      }

      chosenNSFW = [expo, und, outf, ex2, situ, li2, po2, ac2, bo2, nip].filter(Boolean).map(norm);
      p.push(...chosenNSFW);
    ...
    rows.push({
      ...
      neg,
      prompt: p.join(", "),
      text: `${p.join(", ")}${neg?` --neg ${neg}`:""} seed:${seed}`
    });
  }
