...
(function(){
  ...
  function buildColorRe(){
    const pool = new Set((window.SFW?.colors||[]).map(e=>String(e.tag||"").toLowerCase()).filter(Boolean));
    if (pool.size===0) ["orange","blue"].forEach(c=>pool.add(c)); // 念のため
  ...
  window._COLOR_RE = buildColorRe();

  function ensureCatalog(){
    const m = new Map();
    for (const e of (window.SFW?.outfit||[])){
      const tag = e?.tag; const cat = e?.cat;
      if (!tag || !cat) continue;
      m.set(canon(tag), {cat: canon(cat), emit: norm(tag)});
    }
    window.SFW_CATALOG = m;
    console.log('[TEST] catalog size=', m.size);
    return m;
  }

  function getOutfitMeta(token){
    const m = ensureCatalog();
    return m.get(canon(token)) || null;
  }

  window.makeFinalOutfitTags = function(outfits, pc){
    const out = [];
    for (const raw of (outfits||[])){
      ...
      const emit = meta.emit;
      if (meta.cat==="top")      out.push(pc?.top    ? `${pc.top} ${emit}`    : emit);
      else if (meta.cat==="pants"||meta.cat==="skirt")
                                  out.push(pc?.bottom ? `${pc.bottom} ${emit}` : emit);
      else if (meta.cat==="dress")out.push(pc?.top    ? `${pc.top} ${emit}`    : emit);
      else if (meta.cat==="shoes")out.push(pc?.shoes  ? `${pc.shoes} ${emit}`  : emit);
      else out.push(emit);
    }
    return out;
  };
...
// ---- 期待出力の確認 ----
console.log(
  makeFinalOutfitTags(["t-shirt","jeans"], {top:"orange", bottom:"blue", shoes:""})
);
// => ["orange t-shirt","blue jeans"] が出ればOK
